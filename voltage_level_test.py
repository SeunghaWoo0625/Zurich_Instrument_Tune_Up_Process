...
drive_test_pulse = pulse_library.const(length = 50e-9, amplitude=1.0)
...
with exp_test.acquire_loop_rt(uid = "acquire_loop",
    ...
    with exp_test.section(uid = "measure and play", on_system_grid=True, alignment=SectionAlignment.LEFT):
        ...
        exp_test.delay(signal = "measure", time = 80e-9)

exp_calibration = Calibration()
measure_osc = Oscillator(frequency = 20e6)
drive_osc = Oscillator(frequency = 0e6)
...
plt.plot(axis[0]/2, np.imag(data), label="Real (I)", alpha=0.7)
