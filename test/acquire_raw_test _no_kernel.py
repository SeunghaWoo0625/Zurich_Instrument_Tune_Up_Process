...
with test_exp.sweep(uid = "phase_sweep", parameter = phase_sweeper):
    with test_exp.acquire_loop_rt(
        ...
        with test_exp.section(uid="measure_section",on_system_grid=True, alignment=SectionAlignment.RIGHT):
            ...
            test_exp.acquire(signal="acquire_signal", handle = "acquire_handle", length = 300e-9)
...
for i, phase in enumerate(phase_sweeper.values):
    ...
    plt.axvline(x= axis[1][341]/2, color='gray', linestyle='--', linewidth=1, label=f'x = {axis[1][341]/2}')
    plt.xlabel("ns")
    ...
    folder_dir = Path(f"figures/test_no_kernel_7GHz_{time_str}")
    folder_dir.mkdir(parents=True, exist_ok=True)

    plt.savefig(folder_dir / rf"raw_test_{round(phase/np.pi*100)}pi.png")
    plt.show
    real[phase] = np.mean(np.real(data[i][201:342]))
    imag[phase] = np.mean(np.imag(data[i][201:342]))
# %%
plt.figure()
