...
folder_dir = Path(f"figures/test_pulse_arbitrary_{time_str}")
folder_dir.mkdir(parents=True, exist_ok=True)

pulse2_array = pulse_array_test(frequency=50e6, length=200e-9, phase=0, amplitude=.5)
pulse2 = sampled_pulse(samples=pulse2_array, uid="array_pulse")

test_pulse = pulse_library.const(length= 100e-9, amplitude= 0.5)

device = calibrate_devices(measure_type = "inte_square")
...
with test_exp.sweep(uid = "phase_sweep", parameter = phase_sweeper):
    with test_exp.acquire_loop_rt(
        ...
        with test_exp.section(uid="measure_section",on_system_grid=True, alignment=SectionAlignment.LEFT):
            test_exp.play(signal = "measure_signal", pulse = pulse2, phase = phase_sweeper)
